...
void PackMatrixA8x4( int, double *, int, double * );
void PackMatrixB( int, double *, int, double * );
void InnerKernel( int, int, int, double *, int, double *, int, double *, int, int );
void InnerKernel8x4( int, int, int, double *, int, double *, int, double *, int, int );
                                    ...
                                    double *c, int ldc )
{
  ...
  for ( p=0; p<k; p+=kc ){
    ...
    for ( i=0; i<m; i+=mc ){
      ib = min( m-i, mc );
      InnerKernel8x4( ib, n, pb, &A( i,p ), lda, &B(p, 0 ), ldb, &C( i,0 ), ldc, i==0 );
    }
  }
}
...
void InnerKernel8x4( int m, int n, int k, double *a, int lda, 
                                       double *b, int ldb,
                                       double *c, int ldc, int first_time )
{
  int i, j;
//   double 
//     packedA[ m * k ];
  double *packedA = (double *)_mm_malloc(m*k*sizeof(double), 32);

//   static double 
//     packedB[ kc*nb ];    /* Note: using a static buffer is not thread safe... */
  double *packedB = (double *)_mm_malloc(kc*nb*sizeof(double), 32);
  ...
  for ( j=0; j<n; j+=4 ){        /* Loop over the columns of C, unrolled by 4 */
    ...
    for ( i=0; i<m; i+=8 ){        /* Loop over the rows of C */
	 ...
	 one routine (four inner products) */
      if ( j == 0 ) 
	PackMatrixA8x4( k, &A( i, 0 ), lda, &packedA[ i*k ] );
...
void PackMatrixA8x4( int k, double *a, int lda, double *a_to )
{
  int j;
...
void AddDot8x4( int k, double *a, int lda,  double *b, int ldb, double *c, int ldc )
{
  ...
  c_00_c_30_vreg.v = _mm256_load_pd((double *) &(C(0, 0))); 
  c_01_c_31_vreg.v = _mm256_load_pd((double *) &(C(0, 1)));
  c_02_c_32_vreg.v = _mm256_load_pd((double *) &(C(0, 2))); 
  c_03_c_33_vreg.v = _mm256_load_pd((double *) &(C(0, 3))); 
  c_40_c_70_vreg.v = _mm256_load_pd((double *) &(C(4, 0))); 
  c_41_c_71_vreg.v = _mm256_load_pd((double *) &(C(4, 1)));
  c_42_c_72_vreg.v = _mm256_load_pd((double *) &(C(4, 2))); 
  c_43_c_73_vreg.v = _mm256_load_pd((double *) &(C(4, 3))); 
